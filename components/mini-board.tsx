...
export default function MiniBoard({ boardIndex, isActive, winner, isMobile = false }: MiniBoardProps) {
  ...
  if (winner) {
    ...
    ringClass = "ring-2 ring-game-active-border"
  }

  return (
    <motion.div
      className={`grid grid-cols-3 gap-1 p-1 md:p-2 border-2 rounded-md ${bgClass} ${borderClass} ${ringClass} ${
        !isActive && !winner ? "opacity-80" : ""
      } relative soft-shadow ${isMobile ? "mobile-mini-board" : ""}`}
      animate={{
        ...
        {winner && (
          ...
          >
            {winner === "draw" ? (
              ...
              <motion.span
                className={`text-3xl md:text-4xl font-bold ${winner === "X" ? "text-blue-600" : "text-rose-600"} drop-shadow-md`}
